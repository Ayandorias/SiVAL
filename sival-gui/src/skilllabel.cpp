...
#include <QPainter>
#include <sivalgui/skilllabel.hpp>
//// end system includes
 ...
 *
 */
SiVAL::SkillLabel::SkillLabel(const QString &label, QWidget *parent)
:QLabel(parent) {
    m_sLabel = label;
    m_sSep = QString (": ");
    m_bLast = false;
 ...
 *
 */
SiVAL::SkillLabel::~SkillLabel() {
}
void SiVAL::SkillLabel::addInfoText(const QString &key, const QString &value, bool dummy) {
    map.append(qMakePair(key, value));
    m_bDummy = dummy;
}
void SiVAL::SkillLabel::last() {
    m_bLast = true;
}
void SiVAL::SkillLabel::setIcon(const QString &icon) {
    m_pIconRenderer = new QSvgRenderer(icon, this);
}
void SiVAL::SkillLabel::setSeparator(const QString &sep) {
    m_sSep = sep;
}
void SiVAL::SkillLabel::setSubentry(bool entry) {
    m_bSubentry = entry;
    if(m_bSubentry) {
        m_iSubentry = 20;
    }
...
void SiVAL::SkillLabel::paintEvent(QPaintEvent *event) {
